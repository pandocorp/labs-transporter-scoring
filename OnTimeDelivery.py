...
support_file_path=os.path.join(diry,'Transporter_scoring_Combined','SupportFiles','Delivery_Files')
...
model_out_path=os.path.join(diry,'Transporter_scoring_Combined','OutputFiles','Delivery_output')
...
hist_data_ml_dlv=Common_Features.correctdtypes(hist_data_ml_dlv)

 
df=hist_data_ml_dlv.drop(['Indent ID','Arrived At','Created Date','Delivery Date','Dispatched At'],1)
...
DataFrame(df_num.drop(['Target'],1).columns,columns=['Num_Var']).to_csv(os.path.join(support_file_path,'Num_Var_Dlv.csv'),index=False)
DataFrame(df_obj.columns,columns=['Obj_Var']).to_csv(os.path.join(support_file_path,'Obj_Var_Dlv.csv'),index=False)
...
df_lab.insert(0, 'Indent ID',hist_data_ml_dlv['Indent ID']) 
df_lab.shape


pickle.dump(d, open(os.path.join(support_file_path,'label_coder_Dlv.sav'), 'wb'))


df_lab=pd.concat([hist_data_ml_dlv['Created Date'],df_lab],1)


df_lab=OnTimeDelivery.data_prep_lag(df_lab)
 

df_lab.to_csv(os.path.join(processed_datafile_path,'df_lab_Dlv.csv'),index=False)
...
df_lab_ml=df_lab.drop(['creationmonth_continous',
                    ...
                    'Dispatchedweekday', 
                    'Dispatchedday',
                    'time_to_delv','sla_delay_charges'],1)
...
pickle.dump(scaler_X, open(os.path.join(support_file_path,'scaler_X_Dlv.sav'), 'wb'))
pickle.dump(scaler_Y, open(os.path.join(support_file_path,'scaler_Y_Dlv.sav'), 'wb'))
...
best_var_df.to_csv(os.path.join(support_file_path,'variables_best_dlv.csv'),index=False)
...
model_LR=ml_model.Logit(predictor_scaled_matrix,response_scaled_vector,filename ='dlv_prob_LR_model.sav')
...
fd.to_csv(os.path.join(model_out_path,'features_coeff_lab_dlv.csv'),index=False)
...
class plots:
    ...
    def dens_box_plot(df1):
        for i, col in enumerate(df1):
            ...
            plt.savefig(os.path.join(model_out_path,'Desity_Dlv.png'))
#             plt.show()
    ...
    def ROC_curve(y,y_hat,p,label):
        ...
        plt.savefig(os.path.join(model_out_path,'ROC_Dlv.png'))
#         plt.show()
...
plots.dens_box_plot(DataFrame(prob_logit,columns=['Delivery_Prob_Distribution_LR']))
