...
support_file_path=os.path.join(diry,'Transporter_scoring_Combined','SupportFiles','Placement_Files')
...
model_out_path=os.path.join(diry,'Transporter_scoring_Combined','OutputFiles','Placement_output')
...
hist_data_ml_PP=Common_Features.correctdtypes(hist_data_ml_PP)

df=hist_data_ml_PP.drop(['Indent ID','Arrived At','Created Date'],1)
...
DataFrame(df_num.drop(['Target'],1).columns,columns=['Num_Var']).to_csv(os.path.join(support_file_path,'Num_Var_PP.csv'),index=False)
DataFrame(df_obj.columns,columns=['Obj_Var']).to_csv(os.path.join(support_file_path,'Obj_Var_PP.csv'),index=False)
...
df_lab.insert(0, 'Indent ID',hist_data_ml_PP['Indent ID']) 

pickle.dump(d, open(os.path.join(support_file_path,'label_coder_PP.sav'), 'wb'))

df_lab=pd.concat([hist_data_ml_PP['Created Date'],df_lab],1)

df_lab=Placement.data_prep_lag(df_lab)
 
df_lab.to_csv(os.path.join(processed_datafile_path,'df_lab_PP.csv'),index=False)
...
predictor_matrix=df_lab_ml.drop(['Created Date','Indent ID','Target'],1).values
...
pickle.dump(scaler_X, open(os.path.join(support_file_path,'scaler_X_PP.sav'), 'wb'))
pickle.dump(scaler_Y, open(os.path.join(support_file_path,'scaler_Y_PP.sav'), 'wb'))


columns_X=df_lab_ml.drop(['Created Date','Indent ID','Target'],1).columns
scaled_X_df.columns=columns_X

Orders_column=df_lab[['Indent ID']].reset_index().drop(['index'],1)
scaled_df=pd.concat([Orders_column,scaled_X_df,scaled_Y_df],1)#df_lab['Created Date'].reset_index().drop(['index'],1)
...
df_minority_upsampled = resample(df_minority, 
                                 ...
                                 n_samples=int(0.7*len(df_majority)),    # to match majority class
...
best_var_df.to_csv(os.path.join(support_file_path,'variables_best_PP.csv'),index=False)


# ## Work with best Variables only
...
model_LR=ml_model.Logit(predictor_scaled_matrix,response_scaled_vector,filename ='place_prob_LR_model.sav')

fd=pd.DataFrame(df_best_train.drop(['Target'],1).columns,columns=['Features'])

fd['coeff']=(model_LR.coef_)[0]

fd.to_csv(os.path.join(model_out_path,'features_coeff_lab.csv'),index=False)
...
class plots:
    ...
    def dens_box_plot(df1):
        for i, col in enumerate(df1):
            ...
            plt.savefig(os.path.join(model_out_path,'Desity_PP.png'))
            #plt.show()
    ...
    def ROC_curve(y,y_hat,p,label):
        ...
        plt.savefig(os.path.join(model_out_path,'ROC_PP.png'))
        #plt.show()
...
plots.dens_box_plot(DataFrame(1-prob_logit,columns=['Placement_Prob_Distribution_LR']))
